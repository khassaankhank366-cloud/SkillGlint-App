        ...
        {mode === 'email' ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { const phone = prompt('Enter your phone number with country code (e.g., +1234567890):'); if (phone) handleOAuth('phone' as any); }} disabled={oauthLoading !== null} className="flex-1 py-3 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center gap-2 text-white/80 text-sm hover:bg-white/15 transition-all disabled:opacity-50">
...
const HomeView = ({ onProfile, onSettings, onNavigate, profile, session, learning }: { onProfile: () => void; onSettings: () => void; onNavigate: (tab: string, tool?: string) => void; profile: Profile | null; session: Session | null; learning: ReturnType<typeof useLearningProgress> }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const fetchedJobs = (liveJobs.length > 0 ? liveJobs : SAMPLE_JOBS) as { title: string; company: string; pay: string; rating: number }[];
