...
const AuthModal = ({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center px-4 animate-fade-in-up" onClick={loading ? undefined : onClose}>
      <div className="bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={onClose} disabled={loading} className="text-white/40 hover:text-white transition-colors disabled:opacity-40"><X className="w-5 h-5" /></button>
        </div>
        ...
        {mode === 'email' ? (
          ...
          <>
            <button onClick={() => { setMode('email'); setError(''); }} disabled={loading} className="w-full py-3.5 rounded-2xl font-medium text-white transition-all hover:scale-[1.01] bg-white/10 border border-white/10 flex items-center justify-center gap-3 mb-4 disabled:opacity-60 disabled:scale-100">
            ...
            <div className="text-center mt-6 pt-4 border-t border-white/10">
              ...
              <button onClick={() => { setMode('signup'); setError(''); }} disabled={loading} className="w-full py-3.5 rounded-2xl font-semibold text-cyan-400 bg-cyan-400/10 border-2 border-cyan-400/30 hover:bg-cyan-400/20 transition-colors disabled:opacity-60">
                Create New Account
              </button>
            </div>
          </>
...
const SettingsPage = ({ onBack, onSignOut, session, onNavigatePage, profile }: { onBack: () => void; onSignOut: () => void; session: Session | null; onNavigatePage: (page: string) => void; profile: Profile | null }) => {
  ...
  const handleSetPin = async () => {
    ...
    if (error) { setPinError(error); } else { setPinSuccess('PIN set successfully! You will be asked for it at app launch.'); setPin(''); setPinConfirm(''); }
  };

  const handleRemovePin = async () => {
    const { error } = await disablePin(session);
    if (error) { setPinError(error); } else { setPinSuccess('PIN lock removed.'); setPin(''); setPinConfirm(''); }
  };

  const handleSubmitTicket = async () => {
    setTicketStatus('');
    if (!ticketSubject.trim() || !ticketMessage.trim()) { setTicketStatus('Please fill in all fields.'); return; }
    const { error } = await createSupportTicket(session, ticketSubject.trim(), ticketMessage.trim());
    if (error) { setTicketStatus(`Error: ${error}`); } else { setTicketStatus('Support ticket submitted! We will get back to you soon.'); setTicketSubject(''); setTicketMessage(''); }
  };

  const menuItems = [
    { icon: KeyRound, label: 'Security (PIN Lock)', action: () => setView('security') },
    { icon: Shield, label: 'Privacy Policy', action: () => onNavigatePage('privacy') },
    { icon: FileText, label: 'Terms & Policies', action: () => onNavigatePage('terms') },
    { icon: FileDown, label: 'Export My Data', action: () => { exportUserData(session); } },
...
function useLearningProgress(userId: string | undefined, session: Session | null) {
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const c of courses) map[c.title] = c.id;
      setCourseMap(map);
      if (session) {
        const dbProgress = await fetchUserProgress(session);
        setProgress(dbProgress);
      }
    })();
  }, [session, userId]);

  const save = (c: boolean, s: number) => {
    try { localStorage.setItem(storageKey, JSON.stringify({ checkedIn: c, streak: s })); } catch { /* ignore */ }
  };

  const updateCourseProgress = (title: string, value: number) => {
    const courseId = courseMap[title];
    if (courseId) setProgress(p => ({ ...p, [courseId]: Math.min(100, Math.max(0, value)) }));
  };

  const completeCourse = async (title: string) => {
    const courseId = courseMap[title];
    if (!courseId || !session) return;
    const lessons = await fetchLessons(courseId);
    for (const lesson of lessons) {
      await markLessonComplete(session, lesson.id, courseId);
    }
    setProgress(p => ({ ...p, [courseId]: 100 }));
  };
  ...
  return { progress, checkedIn, streak, updateCourseProgress, completeCourse, checkIn, courseMap };
}
...
const HomeView = ({ onProfile, onSettings, onNavigate, profile, session, learning }: { onProfile: () => void; onSettings: () => void; onNavigate: (tab: string, tool?: string) => void; profile: Profile | null; session: Session | null; learning: ReturnType<typeof useLearningProgress> }) => {
  ...
  useEffect(() => {
    (async () => {
      setJobsLoading(true);
      setJobsError('');
      const [liveJobs, coursesRes] = await Promise.all([
        fetchLiveJobs(),
        supabase.from('courses').select('id, title, lessons, progress, icon_name').order('created_at', { ascending: false }),
      ]);
      if (liveJobs.length === 0) {
        setJobsError('Could not fetch live jobs. Check console for details.');
        console.error('[jobs] No jobs returned from API. Check SERPAPI_KEY and JOOBLE_API_KEY in Supabase secrets.');
      }
      const iconByName: Record<string, React.ElementType> = { FileText, Palette, Brain, Trophy, BookOpen };
      const fetchedJobs = liveJobs as { title: string; company: string; pay: string; rating: number }[];
      const courseData = (coursesRes.data && coursesRes.data.length > 0 ? coursesRes.data : SAMPLE_COURSES.map(c => ({ id: '', title: c.title, lessons: c.lessons, progress: 0, icon_name: '' })));
      const fetchedCourses = courseData.map((c: { id: string; title: string; lessons: number; progress: number; icon_name: string }) => {
        const courseId = c.id || '';
        const userProgress = courseId ? (progress[courseId] || 0) : 0;
        return { title: c.title, lessons: c.lessons, progress: userProgress, icon: iconByName[c.icon_name] || (SAMPLE_COURSES.find(sc => sc.title === c.title)?.icon) || BookOpen };
      });
      setJobs(fetchedJobs);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {jobsLoading ? (
              <div className="col-span-full flex flex-col items-center justify-center py-8 gap-2">
                <span className="w-6 h-6 border-2 border-cyan-400/30 border-t-cyan-400 rounded-full" style={{ animation: 'spin 0.6s linear infinite' }} />
                <p className="text-white/50 text-sm">Fetching live jobs...</p>
              </div>
            ) : jobsError ? (
              <div className="col-span-full text-center py-8 text-red-400/70 text-sm">{jobsError}</div>
            ) : jobs.length === 0 ? (
...
const UtilitiesView = ({ onProfile, onSettings, activeTool, setActiveTool }: { onProfile: () => void; onSettings: () => void; activeTool: string; setActiveTool: (v: string) => void }) => {
  ...
  const checkScam = async () => {
    ...
    const isScam = !!found;
    const message = found ? `"${found}" is a KNOWN SCAM SITE. Do NOT engage or share any personal information!` : 'No known scams detected. Always verify independently before engaging.';
    setScamResult({ safe: !isScam, message });
    try {
      const { data: { session: s } } = await supabase.auth.getSession();
      if (s) await reportScam(s, scamSearch, isScam, message);
    } catch { /* ignore */ }
  };
...
const Dashboard = ({ onSignOut, profile, session }: { onSignOut: () => void; profile: Profile | null; session: Session | null }) => {
  ...
  const learning = useLearningProgress(session?.user?.id, session);
...
function App() {
  ...
  if (session && profile?.pin_enabled && profile?.pin_code && !pinVerified) {
    ...
    return (
      <div className="min-h-screen bg-[#0B0F19] flex flex-col items-center justify-center px-6 relative overflow-hidden">
        <SparksBackground />
        <div className="absolute top-1/3 left-1/3 w-[500px] h-[500px] bg-cyan-600/8 rounded-full blur-[150px]" />
        <div className="relative z-10 flex flex-col items-center w-full max-w-xs">
          <div className="w-16 h-16 rounded-2xl bg-cyan-400/15 border border-cyan-400/30 flex items-center justify-center mb-6">
